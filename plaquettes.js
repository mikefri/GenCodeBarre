...
// --- CRÉATION DU SVG DE FLÈCHE (MIS À JOUR ET ÉPAIS) ---
function createArrowSVG() {
    const fullOption = refs.arrowOption.value;
    if (fullOption === 'none' || !fullOption.includes('-')) {
        return null;
    }
    ...
    // Configuration de base pour la couleur et le style
    path.setAttribute("fill", "#000"); 
    path.setAttribute("stroke", "none");
    path.setAttribute("stroke-width", "0");


    // 1. Style TRIANGLE (Rempli)
    if (style === 'triangle') {
        if (dir === 'up') {
            dAttribute = "M 50,20 L 80,180 L 20,180 Z";
        } else if (dir === 'down') {
            dAttribute = "M 50,180 L 80,20 L 20,20 Z";
        }
        
    // 2. Style V (Contour, ouvert)
    } else if (style === 'v') {
        ...
        if (dir === 'up') {
            dAttribute = "M 20,150 L 50,50 L 80,150";
        } else if (dir === 'down') {
            dAttribute = "M 20,50 L 50,150 L 80,50";
        }
        
    // 3. Style LINE (Ligne avec tête de flèche standard, ÉPAISSE)
    } else if (style === 'line') {
        path.setAttribute("fill", "none"); 
        path.setAttribute("stroke", "#000"); 
        path.setAttribute("stroke-width", "16"); // <-- ÉPAISSEUR AUGMENTÉE ICI
        path.setAttribute("stroke-linecap", "round");
...
function createCell(text, scale, containerElement, isDemo = false) {
    ...
    const arrowSVG = createArrowSVG(); // La fonction lit la référence refs.arrowOption.value
    if (arrowSVG) {
